...
import { updateAdAction } from '../adSettings/adSlice';
...
const EditScreen = ({ route, navigation }) => {
  ...
  const handleUpdateAd = () => {
    ...
    dispatch(updateAdAction(updatedAd));
    navigation.navigate('Pagrindinis'); 
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Redaguoti skelbimą</Text>
      <TextInput
        style={styles.input}
      ...
      >
        <Text style={styles.buttonText}>Atnaujinti skelbimą</Text>
      </TouchableHighlight>
      </View>
  );
};
